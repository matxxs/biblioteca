...
export function LivroForm({
  ...
  const allEditoras = useMemo(() => {
    ...
    if (
      livro?.editora &&
      !combined.some((e) => e.editoraID === livro.editora?.editoraID)
    ) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const data: Livro = {
      ...
      anoPublicacao: formData.anoPublicacao
        ? parseInt(formData.anoPublicacao)
        : undefined,
      edicao: formData.edicao,
      numeroPaginas: formData.numeroPaginas
        ? parseInt(formData.numeroPaginas)
        : undefined,
      sinopse: formData.sinopse,
      autores: formData.autores.map((id) => ({
        autorID: parseInt(id),
      })) as Autor[],
      generos: formData.generos.map((id) => ({
        generoID: parseInt(id),
      })) as Genero[],
    };
    onSave(data);
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        <Select
          key={formData.editoraID}
          value={formData.editoraID}
          onValueChange={(value) =>
            setFormData({ ...formData, editoraID: value })
          }
        >
          ...
          <SelectContent>
            {allEditoras.map((editora) => (
              <SelectItem
                ...
                value={editora.editoraID.toString()}
              >
                {editora.nome}
              </SelectItem>
            ))}
              ...
              autores.map((autor) => (
                ...
                >
                  <Checkbox
                    ...
                    checked={formData.autores.includes(
                      autor.autorID.toString()
                    )}
              ...
              generos.map((genero) => (
                ...
                >
                  <Checkbox
                    ...
                    checked={formData.generos.includes(
                      genero.generoID.toString()
                    )}
                    onCheckedChange={(checked) =>
                      handleCheckboxChange(
                        genero.generoID.toString(),
                  ...
                  />
                  <Label
                    htmlFor={`genero-${genero.generoID}`}
                    className="font-normal cursor-pointer"
                  >
                    {genero.nome}
                  </Label>
      ...
      <div className="space-y-2">
        ...
        <Textarea
          ...
          placeholder="Escreva a sinopse do livro aqui..."
          rows={4}
        />
      </div>
